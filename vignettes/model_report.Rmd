...
params: 
  model: ""
  model_dir: "results/evaluated-scenarios-2019-03-12_18:21:04/baseline-2019-03-12_18:21:06"
vignette: >
...
inc_sens <- safe_test_sensitivity(model, obs = "YearlyInc")$results
...
safe_plot_sensitivity(inc_sens)$results
```

- Calculate childhood incidence sensitivity to parameters at the maximum time fitted to.

```{r}
child_inc_sens <- safe_test_sensitivity(model, obs = "YearlyChildInc")$results
...
safe_plot_sensitivity(child_inc_sens)$results
```
...
plot_state(model, states = c("YearlyHistPInc", "YearlyInc", "YearChildInc", "YearlyOlderAdultInc"), summarise = TRUE, start_time = 49) -> p2
...
```{r overview-states-1990-age fig.height = 16, fig.width = 16}
