...
- Future already predicted for baseline scenario (school age vaccination) during model fitting process.
...
plot_state(model, summarise = TRUE, start_time = 69)
```

- Plot fitted observed states.

```{r overview-scenarios-2000-obs}
plot_state(model, states = c("YearlyHistPInc", "YearlyInc"), start_time = 69)
```

- Table fitted states

```{r, eval = FALSE}
pred_obs <- plot_state(model, states = c("YearlyHistPInc", "YearlyInc", "YearlyAgeInc"), summarise = FALSE, start_time = 74, plot_data = FALSE)
