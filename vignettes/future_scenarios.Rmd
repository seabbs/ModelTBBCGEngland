...
```{r overview-scenarios-2000, fig.height = 16, fig.width = 16, cache = TRUE}
plot_state(models, summarise = TRUE, start_time = 69)
```

- Plot fitted observed states.

```{r overview-scenarios-2000-obs, cache = TRUE}
...
```{r overview-scenarios-con-age, cache = TRUE}
plot_state(models, states = c("YearlyAgeInc"), start_time = 69, summarise = TRUE) + 
  theme(legend.position = "none")
```

- Plot age distributed cases.

```{r overview-scenarios-age, fig.height = 16, fig.width = 16, cache = TRUE}
plot_state(models, states = c("YearlyAgeInc"), start_time = 69) + 
  theme(legend.position = "none")
```
