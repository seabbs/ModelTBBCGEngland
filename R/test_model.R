...
#' @param test_fit Logical, defaults to \code{TRUE}. Should the model be fitted with 1000 samples extracted.
...
#' 
#' 
test_model <- function(model= "BaseLineModel", nparticles = 1, adapt_particles = FALSE,
                       adapt_proposal = FALSE, min_acc = 0.05, max_acc = 0.4, test_fit = TRUE,
                       save_output = FALSE, verbose = TRUE, browse = FALSE) {
  ...
  tb_model <- sample(tb_model, target="prior", nsamples=1000, end_time=100*12, noutputs=100, nparticles = nparticles, nthreads = 4)
  ...
  if (test_fit) {
    ...
    tb_model <- tb_model %>% 
      sample(target="posterior",obs=tb_data, sample_obs=TRUE, nsamples=1000, verbose = FALSE, nthreads = 4)
  ...
  if (save_output) {
    ...
    test_data_loc <- paste0("vignettes/results/tests/model-", Sys.Date(), "-", round(runif(1, 0, 10000)), ".rds")
