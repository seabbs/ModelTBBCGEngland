...
```{r load-and-clean-data}
  ...
  #' Filter for only cases that were UK born (as study population)
  ets <- ets %>%
  ...
  ## Add model age groups
    ...
    mutate(age_group = cut(age, c(0, seq(5, 70, 5), 90),
                           ...
                           right = FALSE,
                           ordered_result = TRUE)) %>% 
    mutate(age_stage = cut(age, c(0, 15, 60, 90),
...
```{r check-outliers} 
  ...
  ets %>%
    filter(symptonset < as.Date("1990-12-31")) %>%
...
```{r check-filter-outliers}
  ...
  ets %>%
    filter(starttreatdate < as.Date("1995-12-31")) %>%
...
pul_by_age <- ets %>% 
  ...
  filter(pulmextrapulm %in% "Pulmonary, with or without EP")
...
tb_mortality <- ets %>% 
  ...
  filter(!(finaloutcome12 %in% c("Not Evaluated", "Lost to follow up", "NADeath"))) %>% 
  add_count(age_stage, pulmextrapulm, wt = n) %>% 
  filter(finaloutcome12 %in% "TBDeath") %>% 
  mutate(prop = n /nn,
...
tb_loss_follow <- ets %>% 
  ...
  count(finaloutcome12, age_stage, pulmextrapulm) %>% 
  filter(!(finaloutcome12 %in% c("Not Evaluated"))) %>% 
  add_count(age_stage, pulmextrapulm, wt = n) %>% 
  filter(finaloutcome12 %in% "Lost to follow up") %>% 
  mutate(prop = n /nn,
         sd  = sqrt(prop * (1 - prop) / n))
