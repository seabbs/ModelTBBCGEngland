...
add_row(
  ...
  add_row(
    ...
    Distribution = "$\\alpha^T_{j} = 1 - e^{\\alpha^{\\text{ln}(T)}_{j}}$, where $\\alpha^{\\text{ln}(T)}_{i} = \\mathcal{N}(-1.86, 0.22)$, $\\alpha^{\\text{ln}(T)}_{i+5} = \\mathcal{N}(-1.19, 0.24)$, $ \\alpha^{\\text{ln}(T)}_{i+10} = \\mathcal{N}(-0.84, 0.22)$, $\\alpha^{\\text{ln}(T)}_{i+15} = \\mathcal{N}(-0.84, 0.2)$, $\\alpha^{\\text{ln}(T)}_{i+20} = \\mathcal{N}(-0.28, 0.19)$, $ \\alpha^{\\text{ln}(T)}_{i+25} = \\mathcal{N}(-0.23, 0.29))$ and $i$ is the age group vaccinated",
    Units = "Proportion",
