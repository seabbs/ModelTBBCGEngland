...
log_rr_bcg <- all_rr %>% 
  ...
  mutate_at(vars(log_mean, log_sd), ~ signif(., 4)) %>% 
...
1. Test the log distribution by taking 100,000 samples of the log normal distribution.
...
est_bcg_alpha_decay <- sampled_eff %>% 
  mutate(sample = map(exp_sample, ~ tibble(sample = 1:length(.), value = 1 - .))) %>% 
...
devtools::use_data(est_bcg_alpha_decay, overwrite = TRUE)
 #' Add as raw csv
...
test_decay <- tibble(initial_eff = 1 - exp(rnorm(1e5, mean = log_rr_bcg$log_mean[[1]], sd = log_rr_bcg$log_sd[[1]])),
  ...
  mutate(conf.high = estimate + 1.96 * sd, conf.low = estimate - 1.96 * sd) %>% 
